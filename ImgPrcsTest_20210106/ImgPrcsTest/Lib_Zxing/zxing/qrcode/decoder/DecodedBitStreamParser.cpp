...
void DecodedBitStreamParser::append(std::string &result, const unsigned char *bufIn, size_t nIn, const char *src) {
	...
	wchar_t chData[1024];
	memset(chData,0,sizeof(wchar_t )*1024);
	char chData2[1024];
	memset(chData2,0,sizeof(char )*1024);
	LPWSTR lpwstr = (LPWSTR )chData;
	 MultiByteToWideChar( CP_UTF8, 0, (LPCSTR)bufIn,
        nIn, (LPWSTR)chData,   
     nIn );

	 WideCharToMultiByte( CP_ACP, 0, (LPCWSTR)chData, -1,
        (LPSTR)chData2, 1024, NULL, NULL );

	result.append((const char *)chData2, nIn);
//#ifndef NO_ICONV
//  if (nIn == 0) {
//    return;
//  }
